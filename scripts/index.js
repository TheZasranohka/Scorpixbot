...
const start = async (url, options) => {
    ...
    function sendQuestionsButtonMessage(chatId) {
        const message = 'Ниже представлены самые часто задаваемые вопросы:';
        const quesoptions = {
            reply_markup: {
                inline_keyboard: [
        ...
        if (!pressedQuestions[chatId] || !pressedQuestions[chatId][query.data]) {
            switch (query.data) {
                case 'question_button1':
                    await bot.sendMessage(chatId,'*Что такое «Цена объекта»?* \n' +
                        'Это стоимость объекта недвижимости, который находится на продаже, собранная команда финансирует рекламную кампанию данного объекта.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button2':
                    await bot.sendMessage(chatId,'*Что такое «Рекламный депозит на команду»?* \n' +
                        'Этой суммой оплачивается рекламная кампания, благодаря которой идет активный поиск клиента на покупку рекламируемого объекта недвижимости. Формула: % от общей стоимости объекта.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button3':
                    await bot.sendMessage(chatId,'*Что такое «Количество участников команды»?* \n' +
                        'Максимальное количество мест в команде, Вы можете выбрать участие в той программе, которая Вас больше всего заинтересовала.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button4':
                    await bot.sendMessage(chatId,'*Что такое «Депозит на одного члена команды»?* \n' +
                        'Необходимая сумма для вступления в ту или иную команду (зависит от выбранной Вами программы), рассчитывается пропорционально от общего количества мест в команде.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button5':
                    await bot.sendMessage(chatId,'*Что такое «Доход Scorpix за продажу объекта»?* \n' +
                        'Общая сумма комиссионных выплачиваемых нашей компании за клиента, которого мы нашли благодаря рекламной кампании команды (выплачиваются сразу от общей стоимости объекта, независимо от того куплен ли объект за полную оплату или в рассрочку). Формула: 1.5–5 % от общей стоимости объекта.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button6':
                    await bot.sendMessage(chatId,'*Что такое «Комиссия Scorpix»?* \n' +
                        'Чистый доход нашей компании от общей суммы комиссии, которую получил Scorpix. Вы должны понимать, что мы разделяем нашу комиссию с риелторскими агентствами, работниками нашей компании и оплачиваем все налоги и прочие расходы при подписании контракта на покупку объекта. Формула: 15–40 % от общего дохода за продажу объекта.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button7':
                    await bot.sendMessage(chatId,'*Что такое «Чистая прибыль команды с продажи»?* \n' +
                        'Чистая прибыль в $ на всех участником команды.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button8':
                    await bot.sendMessage(chatId,'*Что такое «Чистая прибыль одного участника команды»?* \n' +
                        'Чистая выручка в $ на одного участника, рассчитывается пропорционально от общего количества участников в команде.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button9':
                    await bot.sendMessage(chatId,'*Что такое «Срок работы командного депозита»?* \n' +
                        'Это максимальное время работы рекламной кампании до осуществления сделки по покупке объекта.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button10':
                    await bot.sendMessage(chatId,'*Где я могу просмотреть объекты, которые рекламируются?*\n' +
                        'Вы можете ознакомиться с объектами, на которые формируются команды в закрытой группе, после разговора с менеджером нашей компании Вас переведут в группу, где формируются новые команды на рекламу объектов.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button11':
                    await bot.sendMessage(chatId,'*Чем можете гарантировать безопасность средств?* \n' +
                        'Каждого нового клиента мы обучаем преимуществу блокчейна, перед пополнением депозита Вы пройдете подготовку у нашего аналитического отдела и научитесь отслеживать Ваш депозит и его передвижение в реальном времени. В личном кабинете проекта вы сможете также отслеживать всю информацию и проверять наличие ваших финансов.', {parse_mode: 'Markdown'})
                    break;
                case 'question_button12':
                    await bot.sendMessage(chatId,'*Есть ли у нашего проекта успешные кейсы и положительные отзывы?* \n' +
                        'Да, есть. Вы сможете пообщаться с действующими клиентами в закрытой группе после общения с нашими менеджерами. Коллективный формат работы является новинкой в сфере недвижимости и рекламы, так что не теряйте время и оставляйте заявку на обратную связь. Мы понимаем, как трудно выгодно вложить небольшую сумму что бы успешно с нее заработать, именно поэтому даем возможность каждому желающему попробовать работать с нами в коллективном формате.', {parse_mode: 'Markdown'})
                    break;
            ...
            if (!pressedQuestions[chatId]) {
                pressedQuestions[chatId] = {};
            }
            pressedQuestions[chatId][query.data] = true;
